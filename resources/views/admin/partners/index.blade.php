{{-- Mewarisi layout admin utama (admin.blade.php) yang ada di folder layouts, dan mengirimkan variabel title --}}
@extends('layouts.admin', ['title' => 'Kelola Partner'])

{{-- Menentukan isi bagian 'content' yang akan ditempel di @yield('content') pada layout utama --}}
@section('content')
<header class="flex items-center justify-between gap-4 mb-10">
    ...
    {{-- Tombol navigasi ke route admin.partners.create (halaman tambah data) --}}
...
<div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
    {{-- Alert untuk menampilkan notifikasi sukses dari session, misalnya 'Partner berhasil ditambahkan' --}}
    @if(session('success'))
    ...
    <div class="border-b bg-slate-50/50 px-8 py-6">
        {{-- Form Pencarian dengan method GET untuk mengirim parameter pencarian 'q' ke URL --}}
        <form method="GET" action="{{ route('admin.partners.index') }}" class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div class="flex w-full gap-3 lg:max-w-md">
                ...
                {{-- Value input diambil dari fungsi request('q') agar kata kunci pencarian yang sebelumnya diinput tetap tampil --}}
                ...
                {{-- Jika ada parameter pencarian aktif, tampilkan tombol Reset untuk mengembalikan ke daftar utama --}}
                @if(request()->filled('q'))
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y border-t">
                {{-- Perulangan @forelse untuk menampilkan daftar partner. Jika data kosong, akan masuk ke blok @empty --}}
                ...
                <tr class="hover:bg-slate-50/50 transition">
                    ...
                    <td class="px-8 py-6">
                        {{-- Menampilkan gambar logo menggunakan URL dari database --}}
                    ...
                    {{-- Format tanggal ke format Indonesia menggunakan method format('d M Y, H:i') --}}
                    ...
                    <td class="px-8 py-6">
                        <div class="flex gap-2">
                            {{-- Tombol Edit yang mengarahkan ke route edit partner dengan parameter objek partner terkait --}}
                            ...
                            {{-- Form Delete dengan method POST yang dimanipulasi dengan @method('DELETE') karena browser hanya mensupport GET dan POST --}}
                            <form action="{{ route('admin.partners.destroy', $partner) }}" method="POST" onsubmit="return confirm('Hapus partner ini?')">
                                {{-- Token keamanan csrf wajib untuk mencegah serangan Cross-Site Request Forgery --}}
                                @csrf
                                @method('DELETE')
                ...
                @empty
                {{-- Ditampilkan jika koleksi $partners kosong --}}
                <tr>
    ...
    {{-- Menampilkan navigasi link pagination (halaman sebelumnya/selanjutnya) --}}
