...
@section('content')
    <main class="mx-auto grid max-w-7xl grid-cols-1 gap-12 px-6 py-12 lg:grid-cols-3">
        ...
        <div class="space-y-12 lg:col-span-2">
            <div class="space-y-4">
                <span class="rounded-full bg-indigo-100 px-4 py-1.5 text-sm font-bold uppercase tracking-wider text-indigo-700">{{ $event->category?->name ?? 'Event' }}</span>
                <h1 class="text-4xl font-black leading-tight md:text-5xl">{{ $event->title }}</h1>
            ...
            <div class="space-y-4">
                ...
                <p class="text-lg leading-relaxed text-slate-600">{{ $event->description }}</p>
            </div>
